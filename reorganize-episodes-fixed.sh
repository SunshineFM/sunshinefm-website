...
echo "🚀 SunshineFM Episode Reorganization (FIXED)"
echo "============================================"

REPO="/Users/macmini/sunshinefm-website"
cd "$REPO"

# Step 3: Generate schema-enhanced index.html for each episode
...
for date_folder in sorted(episodes_base.iterdir()):
    ...
    # Add entities (FIXED: was ep['entities'], now entities)
...
print(f"\n✨ Generated {count} episode pages with RadioEpisode schema")

SCHEMA_SCRIPT

python3 /tmp/generate_schemas.py

# Step 4: Generate updated sitemap.xml
echo ""
echo "🗺️  Step 4: Generating sitemap.xml..."
...
echo ""
echo "Episode structure:"
find "$REPO/episodes/2026/january" -name "index.html" | wc -l
echo "  ↳ HTML files generated (should be 7-8)"
echo ""

echo "✅ COMPLETE! Ready for Steps 6:"
echo "  git add episodes/2026/ sitemap.xml"
echo "  git commit -m 'Reorganize episodes to clean ISO structure with RadioEpisode schema'"
